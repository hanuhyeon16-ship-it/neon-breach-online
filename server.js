...
const AGENTS={
 blitz:{name:'BLITZ',skill:'OVERDRIVE',cooldown:14},
 ward:{name:'WARD',skill:'BARRIER',cooldown:16},
 pulse:{name:'PULSE',skill:'SCAN',cooldown:17},
 mist:{name:'MIST',skill:'SMOKE',cooldown:15}
};
...
function kill(room,t,a,headshot=false){t.dead=true;t.hp=0;t.respawn=2.5;t.d++;if(a){a.k++;if(room.mode!=='control'){if(a.team==='blue')room.blue++;else room.red++}send(a.ws,{type:'hitconfirm',headshot,kill:true});bc(room,{type:'event',event:'kill',killer:a.name,victim:t.name,team:a.team,headshot})}}
function useSkill(room,p,m){if(room.phase!=='playing'||p.dead||p.skillCD>0)return;const a=AGENTS[p.agent];if(!a)return;p.skillCD=a.cooldown;p.lastAction=Date.now();if(p.agent==='blitz'){p.overdrive=4;send(p.ws,{type:'event',event:'skill',skill:'overdrive'})}else if(p.agent==='ward'){const x=p.x+Math.cos(p.a)*95,y=p.y+Math.sin(p.a)*95;room.barriers.push({x,y,w:120,h:28,team:p.team,life:6});bc(room,{type:'event',event:'skill_fx',skill:'barrier',x,y,team:p.team})}else if(p.agent==='pulse'){const until=Date.now()+3000;for(const q of room.players.values())if(q.team!==p.team&&q.team!=='spec')q.scanUntil=Math.max(q.scanUntil||0,until);bc(room,{type:'event',event:'skill_fx',skill:'scan',team:p.team})}else if(p.agent==='mist'){const tx=Number(m.x),ty=Number(m.y),dx=tx-p.x,dy=ty-p.y,d=Math.hypot(dx,dy)||1,dist=Math.min(500,d),x=p.x+dx/d*dist,y=p.y+dy/d*dist;if(!blocked(room,x,y,10))room.smokes.push({x,y,r:185,team:p.team,life:7});bc(room,{type:'event',event:'skill_fx',skill:'smoke',x,y,team:p.team})}}
function fire(room,p,m){const w=WEAPONS[p.weapon];if(room.phase!=='playing'||p.dead||p.reload>0||p.fireCD>0||p.ammo<=0||!w)return;p.fireCD=w.fire;p.ammo--;p.lastAction=Date.now();const now=Date.now(),rewind=Math.min(250,(p.ping||80)/2+60),shotTime=Math.max(now-rewind,Math.min(now,Number(m.ts)||now));const moving=Math.hypot(p.inputX,p.inputY)>.15;const spread=w.spread+(moving?w.moveSpread:0);const rays=[];let anyHead=false,anyHit=false;for(let pellet=0;pellet<w.pellets;pellet++){const ang=(Number(m.a)||p.a)+(Math.random()-.5)*spread*2,dx=Math.cos(ang),dy=Math.sin(ang);let max=w.range,wallDist=rayWallDistance(room,p.x,p.y,dx,dy,max),best=null,bestDist=wallDist;for(const t of room.players.values()){if(t.dead||t.team===p.team||t.team==='spec'||!t.connected)continue;const hp=historyPos(t,shotTime),hit=rayCircle(p.x,p.y,dx,dy,hp.x,hp.y,t.r,max);if(hit&&hit.dist<bestDist){best=t;bestDist=hit.dist;best={target:t,perp:hit.perp,dist:hit.dist}}}let endDist=Math.min(max,bestDist);let head=false;if(best){head=best.perp<=7;const dmg=Math.round(w.damage*(head?w.head:1));best.target.hp-=dmg;anyHit=true;anyHead=anyHead||head;if(best.target.hp<=0)kill(room,best.target,p,head);else send(p.ws,{type:'hitconfirm',headshot:head,kill:false})}rays.push({x1:p.x,y1:p.y,x2:p.x+dx*endDist,y2:p.y+dy*endDist,head})}bc(room,{type:'shot',id:nextTracer++,owner:p.id,team:p.team,weapon:p.weapon,rays})}
function processInput(room,p,m){if(room.phase!=='playing'||p.dead)return;if((p.inputCredit||0)<INPUT_DT*.8)return;p.inputCredit=Math.max(0,(p.inputCredit||0)-INPUT_DT);let dx=Math.max(-1,Math.min(1,Number(m.dx)||0)),dy=Math.max(-1,Math.min(1,Number(m.dy)||0)),l=Math.hypot(dx,dy);if(l>1){dx/=l;dy/=l}p.inputX=dx;p.inputY=dy;p.a=Number(m.a)||0;p.ack=Math.max(p.ack,Number(m.seq)||0);if(Math.hypot(dx,dy)>.01)p.lastAction=Date.now();if(m.dash&&p.dashCD<=0){let xx=dx,yy=dy;if(Math.hypot(xx,yy)<.01){xx=Math.cos(p.a);yy=Math.sin(p.a)}l=Math.hypot(xx,yy)||1;p.dashX=xx/l;p.dashY=yy/l;p.dashTime=DASH_TIME;p.dashCD=DASH_CD;p.lastAction=Date.now()}let speedMul=p.overdrive>0?1.15:1;let vx,vy;if(p.dashTime>0){p.dashTime=Math.max(0,p.dashTime-INPUT_DT);vx=p.dashX*DASH_SPEED*speedMul;vy=p.dashY*DASH_SPEED*speedMul}else{vx=dx*WALK*speedMul;vy=dy*WALK*speedMul}moveSub(room,p,vx,vy,INPUT_DT)}
function tickRoom(room,dt){const now=Date.now();for(const p of room.players.values()){p.inputCredit=Math.min(.10,(p.inputCredit||0)+dt);p.fireCD=Math.max(0,p.fireCD-dt);p.dashCD=Math.max(0,p.dashCD-dt);p.skillCD=Math.max(0,p.skillCD-dt);p.overdrive=Math.max(0,(p.overdrive||0)-dt);if(p.reload>0){p.reload-=dt;if(p.reload<=0){const w=WEAPONS[p.weapon],need=w.mag-p.ammo,take=Math.min(need,p.reserve);p.ammo+=take;p.reserve-=take}}if(p.team!=='spec'){p.history.push({t:now,x:p.x,y:p.y});while(p.history.length&&now-p.history[0].t>400)p.history.shift()}if(p.connected&&room.phase==='playing'&&now-p.lastAction>AFK_WARN_MS&&!p.afkWarned){p.afkWarned=true;send(p.ws,{type:'event',event:'afk_warning'})}if(p.connected&&room.phase==='playing'&&now-p.lastAction>AFK_SPEC_MS){p.team='spec';p.dead=true;p.ready=false;bc(room,{type:'event',event:'afk_spectate',name:p.name})}}
 ...
 room.time-=dt;const md=MODES[room.mode];for(const p of activePlayers(room))if(p.dead&&md.respawn){p.respawn-=dt;if(p.respawn<=0)spawn(room,p)}
...
wss.on('connection',ws=>{const id=nextId++;let room=null,p=null;send(ws,{type:'hello',id,version:VERSION,weapons:WEAPONS,agents:AGENTS,modes:MODES,maps:MAPS});ws.on('message',raw=>{let m;try{m=JSON.parse(raw)}catch{return}
 ...
 });ws.on('close',()=>{if(!room||!p)return;p.connected=false;p.disconnectAt=Date.now();bc(room,{type:'event',event:'disconnected',name:p.name});state(room)})});
