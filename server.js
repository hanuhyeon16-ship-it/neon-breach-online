...
function processInput(room,p,m){if(room.phase!=='playing'||p.dead)return;const incomingSeq=Math.max(0,Number(m.seq)||0);p.ack=Math.max(p.ack,incomingSeq);if((p.inputCredit||0)<INPUT_DT*.8)return;p.inputCredit=Math.max(0,(p.inputCredit||0)-INPUT_DT);let dx=Math.max(-1,Math.min(1,Number(m.dx)||0)),dy=Math.max(-1,Math.min(1,Number(m.dy)||0)),l=Math.hypot(dx,dy);if(l>1){dx/=l;dy/=l}p.inputX=dx;p.inputY=dy;p.a=Number(m.a)||0;if(Math.hypot(dx,dy)>.01)p.lastAction=Date.now();if(m.dash&&p.dashCD<=0){let xx=dx,yy=dy;if(Math.hypot(xx,yy)<.01){xx=Math.cos(p.a);yy=Math.sin(p.a)}l=Math.hypot(xx,yy)||1;p.dashX=xx/l;p.dashY=yy/l;p.dashTime=DASH_TIME;p.dashCD=DASH_CD;p.lastAction=Date.now()}let speedMul=p.overdrive>0?1.15:1;let vx,vy;if(p.dashTime>0){p.dashTime=Math.max(0,p.dashTime-INPUT_DT);vx=p.dashX*DASH_SPEED*speedMul;vy=p.dashY*DASH_SPEED*speedMul}else{vx=dx*WALK*speedMul;vy=dy*WALK*speedMul}moveSub(room,p,vx,vy,INPUT_DT)}
function tickRoom(room,dt){const now=Date.now();for(const p of room.players.values()){p.inputCredit=Math.min(.25,(p.inputCredit||0)+dt);p.fireCD=Math.max(0,p.fireCD-dt);p.dashCD=Math.max(0,p.dashCD-dt);p.skillCD=Math.max(0,p.skillCD-dt);p.overdrive=Math.max(0,(p.overdrive||0)-dt);if(p.reload>0){p.reload-=dt;if(p.reload<=0){const w=WEAPONS[p.weapon],need=w.mag-p.ammo,take=Math.min(need,p.reserve);p.ammo+=take;p.reserve-=take}}if(p.team!=='spec'){p.history.push({t:now,x:p.x,y:p.y});while(p.history.length&&now-p.history[0].t>400)p.history.shift()}if(p.connected&&room.phase==='playing'&&now-p.lastAction>AFK_WARN_MS&&!p.afkWarned){p.afkWarned=true;send(p.ws,{type:'event',event:'afk_warning'})}if(p.connected&&room.phase==='playing'&&now-p.lastAction>AFK_SPEC_MS){p.team='spec';p.dead=true;p.ready=false;bc(room,{type:'event',event:'afk_spectate',name:p.name})}}
